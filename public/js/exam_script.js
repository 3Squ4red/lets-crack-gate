...
function resumeSavedExam() {
    ...
    try {
        ...
        if (examData.isUntimed) {
            timeRemaining = Infinity;
            examEndTime = Infinity;
        } else {
...
if (!window._examScriptLoaded) {
    window._examScriptLoaded = true;
    ...
    if (exitBtn) {
        exitBtn.onclick = function () {
            const doExit = () => {
                if (confirm('Are you sure you want to exit the exam? All your progress will be lost.')) {
                    ...
                    attemptSequence = [];
                    _sectionOffsets = [];
                    _cachedQuestionPaperHTML = null;
                    questionStartTime = null;
                    questionStartGlobalIndex = null;
                    document.querySelectorAll('.exam-item').forEach(function(el) {
                        el.style.opacity = '';
                        el.style.pointerEvents = '';
                    });
                    window._activeExamEl = null;
...
function startTimer() {
    ...
    timer = setInterval(() => {
        ...
        if (examData.isUntimed) {
            updateQuestionTimerDisplay(); // still update question timer
...
function updateTimerDisplay() {
    if (examData && examData.isUntimed) {
        document.getElementById('timeDisplay').textContent = "Untimed";
        return;
    }
