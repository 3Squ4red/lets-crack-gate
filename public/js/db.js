...
const DB_VERSION = 10; // Bumped to match existing browser DB version
...
async function saveBookmarkToDB(bookmarkData) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // Look up existing record by questionHash to avoid ConstraintError on unique index
        const index = store.index('questionHash');
        const lookupReq = index.get(bookmarkData.questionHash);

        lookupReq.onsuccess = () => {
            if (lookupReq.result) {
                // Reuse existing primary key so put() updates instead of creating a duplicate
                obj.id = lookupReq.result.id;
            }
            const writeReq = store.put(obj);
...
async function saveSillyMistakeToDB(data) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // Look up existing record by questionHash to avoid ConstraintError on unique index
        const index = store.index('questionHash');
        const lookupReq = index.get(data.questionHash);
