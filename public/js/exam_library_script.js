...
(async function () {
    ...
    // Expose so result page / index.html can trigger a refresh after exam completion
    window.refreshLibraryAttempts = async function () {
        await loadAttemptData();
        render();
    };
                ...
                html += `
                    <div class="tree-folder" data-name="${escapeAttr(folder.name)}">
                        <div class="folder-header" style="padding-left: ${28 + depth * 16}px" onclick="toggleFolder(this)">
                            ...
                            <span class="folder-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:var(--p)"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg></span>
    ...
    window.toggleMultiSelect = function () {
        ...
        // Save which source cards and folders are currently open
        const openSources = new Set();
        const openFolders = new Set();
        document.querySelectorAll('.source-card.open').forEach(el => openSources.add(el.dataset.sourceIdx));
        document.querySelectorAll('.tree-folder.open').forEach(el => openFolders.add(el.dataset.name));
        ...
        document.querySelectorAll('.source-card').forEach(el => {
            if (openSources.has(el.dataset.sourceIdx)) el.classList.add('open');
        });
        document.querySelectorAll('.tree-folder').forEach(el => {
            if (openFolders.has(el.dataset.name)) el.classList.add('open');
        });
    };
    ...
    function updateFabUI() {
        ...
        if (fabAvailableCount) {
            if (selectedExams.size > 0) {
                ...
                // Always auto-update to current max
    ...
    function startExamDirectly(data, loadingEl) {
        const untimedToggle = document.getElementById('untimedModeToggle');
        if (untimedToggle && untimedToggle.checked) {
            data.isUntimed = true;
        }

        window.examData = data;
        // Restore exam item appearance immediately
        if (loadingEl && loadingEl.id !== "fabGenerateBtn") {
    ...
    function getCategoryIcon(cat) {
        const icons = {
            pyq: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>',
            test_series: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>',
            subject: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>',
            practice: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg>',
            other: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>'
        };
